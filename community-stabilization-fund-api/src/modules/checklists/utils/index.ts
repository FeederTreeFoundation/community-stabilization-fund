...
export const mapFormResponseToBagItems = ({ household_members, feminine_health_care_id }: FormResponse) => {
  return({
    "Groceries": [
      ...
      { name: "Canned Green Beans(baked/black)", quantity: household_members <= 3 ? 2 : 4 },
      { name: "Assorted Fruit 1", quantity: household_members <= 3 ? 4 : 8 },
      { name: "Assorted Fruit 2", quantity: household_members <= 3 ? 4 : 8 },
      { name: "Vegetable Assortment", quantity: household_members <= 3 ? 4 : 8 }
    ],
    "General Hygiene": [
